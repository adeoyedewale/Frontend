pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                git url: 'https://github.com/adeoyedewale/Frontend.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Dockerize') {
            steps {
                sh "docker build -t eruobodo/frontend-app:${BUILD_NUMBER} ."
            }
        }
        //stage('Publish') {
        //    steps {
        ...
        stage('Push') {
              ...
              steps {
                 sh 'docker push eruobodo/frontend-app:${BUILD_NUMBER}'
              }
        }
		}
